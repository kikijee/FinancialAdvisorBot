...
def stock_price():
  ticker = input("Enter ticker for price: ")
  data = web.DataReader(ticker)
  data_t = '%.2f' % data['Close'].iloc[-1]
  print(f"Latest {ticker} price: ${data_t}")

def stock_prediction():
  ticker = input("Enter ticker for price prediction: ")
  data = spm.portfolio_predict(ticker)
  data_t = '%.2f' % data
  print(f"Price prediction of {ticker} for tomorrow: ${data_t}")
